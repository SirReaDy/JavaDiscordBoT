...
public class guildJoinListener extends ListenerAdapter {
    ...
    public void onGuildMemberJoin(GuildMemberJoinEvent event) {
        ...
        if (!SSSS.getSERVERJOINMESSAGE(event.getGuild()).toLowerCase().equals("off")) {
            event.getGuild().getTextChannelsByName("enter/leave", true).get(0).sendMessage(
        ...
        if (!SSSS.getAUTOROLE(event.getGuild()).equals("")) {
            ...
            try {
                pc.sendMessage("*And here a nice cat pic. ^^*\n" + commands.chat.Cat.readJsonFromUrl("http://aws.random.cat/meow").get("file").toString()).queue();
            } catch (Exception e) {
