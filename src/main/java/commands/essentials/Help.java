...
//TODO : FIX THE LAMDA ON HELP COMMAND
...
public class Help implements Command {
    ...
    public Game action(String[] args, MessageReceivedEvent event) {
        ...
        Arrays.stream(ignorers).forEach(s -> {
            cmds.remove (s);
        });
        ...
        try {
            ...
            cmds.keySet().stream()
                    .filter(s -> Main.commands.get(s).equals(STATICS.CMDTYPE.chatutils))
                    .forEach(s1 -> ciams.append(
                            getPermPre(Main.commands.get(s1).permission()) + "  **" + s1 + "**   -   `" + cmds.get(s1) + "`\n"
                    ));
            ...
            cmds.keySet().stream()
                    .filter(s -> Main.commands.get(s).equals(STATICS.CMDTYPE.essentials))
                    .forEach(s1 -> ciams.append(
                            getPermPre(Main.commands.get(s1).permission()) + "  **" + s1 + "**   -   `" + cmds.get(s1) + "`\n"
                    ));
            ...
            cmds.keySet().stream()
                    .filter(s -> Main.commands.get(s).equals(STATICS.CMDTYPE.etc))
                    .forEach(s1 -> ciams.append(
                            getPermPre(Main.commands.get(s1).permission()) + "  **" + s1 + "**   -   `" + cmds.get(s1) + "`\n"
                    ));
            ...
            cmds.keySet().stream()
                    .filter(s -> Main.commands.get(s).equals(STATICS.CMDTYPE.guildadmin))
                    .forEach(s1 -> ciams.append(
                            getPermPre(Main.commands.get(s1).permission()) + "  **" + s1 + "**   -   `" + cmds.get(s1) + "`\n"
                    ));
            ...
            cmds.keySet().stream()
                    .filter(s -> Main.commands.get(s).equals(STATICS.CMDTYPE.music))
                    .forEach(s1 -> ciams.append(
                            getPermPre(Main.commands.get(s1).permission()) + "  **" + s1 + "**   -   `" + cmds.get(s1) + "`\n"
                    ));
            ...
            cmds.keySet().stream()
                    .filter(s -> Main.commands.get(s).equals(STATICS.CMDTYPE.settings))
                    .forEach(s1 -> ciams.append(
                            getPermPre(Main.commands.get(s1).permission()) + "  **" + s1 + "**   -   `" + cmds.get(s1) + "`\n"
                    ));
