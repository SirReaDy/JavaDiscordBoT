...
public class Bug implements Command {
    ...
    public Game action(String[] args, MessageReceivedEvent event) throws ParseException, IOException {
        ...
        try {
            ...
            CHANNEL = event.getJDA().getGuildById("560411426720841739").getTextChannelById("561280901619908620");

        } catch (Exception e) {

            if (args.length < 1) {
                event.getTextChannel().sendMessage(new EmbedBuilder().setDescription(
                        "If you want to submit a bug or a suggestion, please use the public **[google sheet](###)**."
                ).build()).queue();
                return null;
            }
        ...
        if (args.length < 1)
            event.getTextChannel().sendMessage(new EmbedBuilder().setDescription(
                    ...
                    "<bug/gussestion>\n" +
                    "<message>```\n\n" +
                    "Otherwise you can also just use the public **[google sheet](###)** to submit a bug or suggestion."
            ).build()).queue();
        else {
