...
public class Music implements Command {
    ...
    private static final Map<String, Map.Entry<AudioPlayer, audioCore.TrackManager>> players = new HashMap<> ();
    ...
    private audioCore.TrackManager getTrackManager(Guild guild) {
    ...
    private AudioPlayer createPlayer(Guild guild) {
        ...
        audioCore.TrackManager manager = new TrackManager (nPlayer);
    ...
    private void loadTrackNext(String identifier, Member author, Message msg) {
        ...
        myManager.loadItemOrdered(guild, identifier, new AudioLoadResultHandler () {
            ...
            public void trackLoaded(AudioTrack track) {
                ...
                audioCore.AudioInfo currentTrack = getTrackManager(guild).getQueue().iterator().next();
                Set<audioCore.AudioInfo> queuedTracks = getTrackManager(guild).getQueue();
    ...
    private boolean isCurrentDj(Member member) {
        return getTrackManager(member.getGuild()).getInfo (getPlayer(member.getGuild()).getPlayingTrack()).getAuthor().equals(member);
    }
    ...
    private String buildQueueMessage(audioCore.AudioInfo info) {
    ...
    private AudioEventListener audioEventListener = new AudioEventAdapter () {
        ...
        public void onTrackStart(AudioPlayer player, AudioTrack track) {
            if (guild.getTextChannelsByName(SSSS.getMUSICCHANNEL(guild), true).size() > 0) {
                ...
                Set<audioCore.AudioInfo> queue = getTrackManager(guild).getQueue();
                ArrayList<audioCore.AudioInfo> tracks = new ArrayList<>();
        ...
        public void onTrackEnd(AudioPlayer player, AudioTrack track, AudioTrackEndReason endReason) {
            ...
            if (getTrackManager(guild).getQueue ().size() < 2 && endlessMode) {
        ...
        switch (args.length) {
            ...
            case 2:
                switch (args[0].toLowerCase()) {
                    ...
                    case "channel":
                        if (core.Perms.check(2, event)) return null;
                        if (args.length < 2) {
                    ...
                    case "queue":
                        if (!hasPlayer(guild) || getTrackManager(guild).getQueue().isEmpty()) {
                            ...
                            Set<AudioInfo> queue = getTrackManager(guild).getQueue ();
                    ...
                    case "skip":
                        if (isCurrentDj(event.getMember()) || isDj(event.getMember())) {
                            for (int skip = (args.length > 1 ? Integer.parseInt(args[1]) : 1); skip > 0; skip--) {
                                if (isIdle(guild, event)) return null;
                    ...
                    case "endless":
                        ...
                        getTrackManager(guild).getQueue().stream().skip(1).forEach(t -> endlessList.add(t.getTrack()));
                        endlessMode = true;
                        ...
                        try {
                            ...
                            new Timer().schedule(
                                    new java.util.TimerTask() {
                                        @Override
                                        public void run() {
                                            int tracks = getTrackManager(guild).getQueue().size();
                                            event.getTextChannel().sendMessage(
